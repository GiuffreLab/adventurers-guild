...
import { AOE_SKILL_IDS } from './combatlog.js';
...
// ── Per-Class Page ───────────────────────────────────────────────────────
const CLASS_ROLES = {
  HERO: { role: 'All-Rounder / Party Leader', desc: 'The Hero is the backbone of every guild. A versatile fighter who can fill any role through specializations (Vanguard, Champion, or Warden), and whose base kit provides party-wide ATK, DEF, and SPD buffs. Rally Cry reactively heals wounded allies, Sword Dance cleaves all enemies, and the L18 capstone Starfall Slash delivers devastating burst damage. The Hero isn\'t the best tank, healer, or DPS — but they\'re the best <em>second choice</em> for any of those roles, and their leadership makes the whole party stronger.' },
  KNIGHT: { role: 'Tank / Party Protector', desc: 'The Knight is the definitive wall. Massive HP and DEF scaling, with Bulwark intercepting attacks aimed at allies and Shield Wall reducing incoming damage for the whole party. Sweeping Blow provides AoE, Last Stand triggers a defensive surge when wounded, and the Unbreakable capstone survives killing blows. Improved base SPD means Knights now contribute more consistently to the ATB rotation rather than sitting idle between their defensive triggers.' },
  MAGE: { role: 'Magic DPS / AoE Destruction', desc: 'The Mage deals the highest sustained magic damage in the game. A glass cannon with devastating power multipliers and MAG stacking. Blizzard and Meteor Storm provide powerful AoE, and the Arcane Construct pet adds sustained damage. Mage-only weapons from Epic tier onward all carry AoE damage procs (Void Burst → Cataclysm → Arcanum), making the Mage one of the two primary AoE classes alongside the Ranger. Offhand orbs provide buff and aura procs instead, empowering the Mage\'s spell potency.' },
  ROGUE: { role: 'Physical DPS / Crit & DoT Specialist', desc: 'The Rogue is the burst damage king. Highest SPD and crit chance in the game. Mark for Death lets the whole party pile onto a target, Fan of Knives applies AoE poison and exposure, and Riposte counter-strikes when hit. Class-exclusive offhand daggers from Rare tier onward deliver escalating poison DoTs (Envenom → Deadly Toxin → Neurotoxin) that corrode enemy ATK while ticking damage — a unique identity that pairs burst crits with sustained pressure. Shadow Network gives universal SPD, CRIT, and DODGE to allies.' },
  CLERIC: { role: 'Healer / Sustain Anchor', desc: 'The Cleric keeps the party alive with sustained healing, revives, and defensive auras. Divine Intervention saves allies from death, Consecration deals AoE damage while buffing the party, and Righteous Burn adds a fire DoT to the Cleric\'s arsenal. Guardian\'s Grasp negates incoming hits entirely. Improved base SPD means the Cleric now heals more frequently in the ATB rotation, and class weapons carry SPD bonuses from Epic tier to keep pace with the party. The Celestial weapon grants Sanctified Radiance — a persistent party regen at ~70% of base Regen Song potency, giving the Cleric their own sustained healing layer independent of the Bard. Sanctuary provides universal stats (HP, DEF, SPD, CRIT) so every party member benefits.' },
  RANGER: { role: 'Ranged DPS / AoE & Utility', desc: 'The Ranger combines strong physical damage with party utility. Volley and Arrow Storm provide devastating AoE, Precision Shot marks enemies for bonus damage from all sources, and Camouflage grants evasion for survivability. Ranger-only bows from Epic tier onward all carry AoE damage procs (Storm Volley → Celestial Volley → Celestial Barrage), making the Ranger one of the two primary AoE classes alongside the Mage. Offhand quivers provide buff and aura procs instead — speed, precision, and party-wide empowerment. Nature Bond gives party-wide dodge and bonus gold/exp, making Rangers the best class for farming.' },
  BARD: { role: 'Buffer / Songweaver / Wardrum', desc: 'The Bard is the supreme party force multiplier with a dual weapon identity. Regen Song provides persistent HP regen, Discord debuffs enemy ATK and applies sonic DoT, Crescendo grants guaranteed crits, and Cadence buffs the whole party\'s damage and crit. The capstone Sonic Boom delivers burst. The Bard offers a true gear identity fork: <em>Lutes</em> for the Songweaver path (amplified Regen Song potency and party heal received bonus — a healing force multiplier that empowers Cleric heals too), or <em>Drums</em> for the Wardrum path (amplified Discord DoT, stronger ATK reduction, and higher fumble chance — a debuff specialist that cripples enemy output). High DODGE makes them surprisingly survivable — they\'re the strongest "always include" after the Hero.' },
  MONK: { role: 'Hybrid Melee / Reactive Counter-Fighter', desc: 'The Monk is a perfectly balanced bruiser with the game\'s deepest reactive combat loop. Hundred Fists pummels all enemies, Flowing Strike counter-attacks on dodge and triggers Iron Stance (+DEF, +dodge), and Pressure Point debuffs enemy speed and defense while generating Ki Shields. The Monk offers a true gear identity fork: <em>1H Claws + Offhand Claws</em> for aggressive pressure (damage procs + chi burn DoTs that slow enemy ATB), or <em>2H Quarterstaves</em> for the Flowing Defense path (deflect incoming attacks, reflect damage back, and at Celestial tier extend that protection to the whole party). Ki Resonance gives the party an equal spread of ATK, DEF, SPD, and CRIT.' },
  NECROMANCER: { role: 'Summoner / Drain Caster / Death Lord', desc: 'The Necromancer commands the dead and siphons the living. Raise Dead produces thralls that deal automatic damage each round and serve as a Forgo Death shield. Shadow Bolt blasts a single target while healing every ally, and Blight ticks AoE damage over time. The capstone Army of the Damned raises every fallen enemy at once. The Necromancer offers a true gear identity fork: <em>1H Staves + Offhand Skulls/Grimoires</em> for the Drain Caster path (lifetap procs, self/party sustain, stronger personal spells), or <em>2H Scythes</em> for the Death Lord path (deterministic thrall empowerment auras that scale minion damage and HP, amplify Blight, and at Celestial tier grant thrall AoE attacks). Shroud of Decay reflects necrotic damage back at enemies who strike allies.' },
};
    ...
    return skillIds.map(skId => {
      ...
      return `
        <div class="comp-item-row">
          ...
          <span class="comp-item-skill">${skill.icon || '•'} ${skill.name}${AOE_SKILL_IDS.has(skId) ? ' <span class="comp-aoe-badge">AoE</span>' : ''}</span>
