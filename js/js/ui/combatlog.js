...
const T_HEAL_BARD = [
  (m, hp) => `${m} plays a soothing melody — party recovers <span class="dmg-num dmg-heal">+${hp}</span> HP!`,
  (m, hp) => `${m} sings a restorative ballad, healing for <span class="dmg-num dmg-heal">+${hp}</span> HP!`,
];
const T_HEAL = [
  (m, hp) => `${m} channels healing energy — party recovers <span class="dmg-num dmg-heal">+${hp}</span> HP!`,
  (m, hp) => `A warm glow from ${m} heals <span class="dmg-num dmg-heal">+${hp}</span> HP!`,
];
const CLASS_HEAL_TEMPLATES = { CLERIC: T_HEAL_CLERIC, BARD: T_HEAL_BARD };
function getHealTemplate(classId, seed) {
  const templates = CLASS_HEAL_TEMPLATES[classId] || T_HEAL;
  return sPick(templates, seed);
}
const T_DEFEND = [
...
function buildSimulation(aq, quest) {
  ...
  for (let i = 0; i < MAX_BATTLE_EVENTS; i++) {
    ...
    if (roll < 0.35) {
      ...
      const healClassId = healer.class || 'HERO';
      text = getHealTemplate(healClassId, es)(healer.name, healAmt);
      icon = '💚'; type = 'heal';
